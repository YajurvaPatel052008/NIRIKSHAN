...
export default function ProductRepositoryPage() {
  ...
  const filtered = useMemo(() => products.filter((product) => {
    ...
    const lastInspected = product[8] ? new Date(product[8]) : null;
    const days = dateRange === "Last 7 days" ? 7 : dateRange === "Last 30 days" ? 30 : dateRange === "Last 90 days" ? 90 : null;
    const withinDateRange = !days || (lastInspected && (Date.now() - lastInspected.getTime()) <= days * 86400000);
    return (!query || searchable.includes(query.toLowerCase()) || product[0].includes(query.toLowerCase()))
      ...
      && withinDateRange;
  }), [products, query, category, status, manufacturer, dateRange]);
