...
const products = [
  ["prod-001", "Parle-G Biscuits 800 g", "Parle Products Pvt. Ltd.", "Food & Beverages", "10 Sep 2026", "Compliant", 5, "biscuits"],
  ["prod-002", "Lux Soft Touch Soap 100 g", "Hindustan Unilever Ltd.", "Cosmetics", "09 Sep 2026", "Violation", 3, "soap"],
  ["prod-003", "Kurkure Masala Munch 90 g", "PepsiCo India Holdings", "Food & Beverages", "08 Sep 2026", "Pending", 2, "snacks"],
  ["prod-004", "Bisleri Mineral Water 1 L", "Bisleri International Pvt. Ltd.", "Food & Beverages", "07 Sep 2026", "Compliant", 8, "water"],
  ["prod-005", "Surf Excel Matic 2 kg", "Hindustan Unilever Ltd.", "Household", "05 Sep 2026", "Violation", 4, "household"],
  ["prod-006", "Dabur Red Toothpaste 200 g", "Dabur India Ltd.", "Cosmetics", "03 Sep 2026", "Compliant", 6, "personal"],
  ["prod-007", "Tata Salt Iodised 1 kg", "Tata Consumer Products", "Food & Beverages", "01 Sep 2026", "Pending", 2, "salt"],
  ["prod-008", "Amul Taaza Milk 1 L", "Gujarat Co-operative Milk Marketing", "Food & Beverages", "29 Aug 2026", "Compliant", 7, "milk"],
];
...
const manufacturers = ["All manufacturers", ...new Set(products.map((product) => product[2]))];
...
export default function ProductRepositoryPage() {
  ...
  const filtered = useMemo(() => products.filter((product) => {
    const searchable = product.slice(1, 4).join(" ").toLowerCase();
    return (!query || searchable.includes(query.toLowerCase()) || product[0].includes(query.toLowerCase()))
      && (category === categories[0] || product[3] === category)
      && (status === statuses[0] || product[5] === status)
      && (manufacturer === manufacturers[0] || product[2] === manufacturer);
  }), [query, category, status, manufacturer]);
