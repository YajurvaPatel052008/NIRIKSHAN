    ...
    <div className="min-h-screen bg-[#f4f8fa] text-slate-800">
      ...
      <div className="lg:pl-64">
        ...
        <main className="mx-auto max-w-[1350px] p-5 sm:p-8">
          ...
          <div className="mb-6 flex gap-1 overflow-x-auto rounded-lg border border-slate-200 bg-white p-1 shadow-sm">{["Summary", "Declarations", "Violations", "Evidence"].map((label, index) => <button key={label} type="button" onClick={() => setActivePage(index + 1)} className={`whitespace-nowrap rounded-md px-4 py-2 text-xs font-bold ${activePage === index + 1 ? "bg-[#0f3d63] text-white" : "text-slate-500 hover:bg-slate-50"}`}>Page {index + 1} · {label}</button>)}</div>
          {activePage === 1 && <section className="rounded-xl border border-slate-200 bg-white p-6 shadow-sm sm:p-8">
          ...
          {activePage === 2 && <section className="rounded-xl border border-slate-200 bg-white p-6 shadow-sm sm:p-8"><p className="text-xs font-bold uppercase tracking-[0.16em] text-[#168cae]">OCR + semantic extraction</p><h2 className="mt-1 text-xl font-bold text-[#0f3d63]">Extracted Declarations</h2><div className="mt-5 overflow-x-auto"><table className="w-full min-w-[620px] text-left text-sm"><thead><tr className="border-b border-slate-100 text-[10px] uppercase tracking-wider text-slate-400"><th className="px-3 py-3 font-bold">Declaration Type</th><th className="px-3 py-3 font-bold">Extracted Value</th><th className="px-3 py-3 font-bold">Confidence</th><th className="px-3 py-3 font-bold">Status</th></tr></thead><tbody>{data.declarations.map((item, index) => <tr key={item.id || `${item.declaration_type}-${index}`} className="border-b border-slate-50"><td className="px-3 py-4 font-bold text-slate-600">{item.declaration_type}</td><td className="px-3 py-4 text-slate-600">{item.normalized_value || item.extracted_value || "Not detected"}</td><td className="px-3 py-4">{Math.round(item.confidence_score > 1 ? item.confidence_score : (item.confidence_score || 0) * 100)}%</td><td className="px-3 py-4">{item.normalized_value ? <span className="font-semibold text-emerald-700">Found</span> : <span className="font-semibold text-red-600">Not found</span>}</td></tr>)}</tbody></table>{data.declarations.length === 0 && <p className="p-6 text-center text-sm text-slate-500">No declarations were returned.</p>}</div></section>}
          {activePage === 4 && <section className="rounded-xl border border-slate-200 bg-white p-6 shadow-sm sm:p-8">
            <div>
            ...
            {data.violations.filter((violation) => violation.evidence_image_url).length > 0 && <div className="mt-6 grid gap-4 sm:grid-cols-2">{data.violations.filter((violation) => violation.evidence_image_url).map((violation, index) => <figure key={violation.id || `${violation.declaration_type}-${index}`} className="rounded-lg border border-slate-200 p-3"><img src={violation.evidence_image_url} alt={`${violation.declaration_type || "Violation"} evidence`} className="h-32 w-full rounded bg-slate-50 object-contain" /><figcaption className="mt-2 text-xs font-semibold text-[#0f3d63]">{violation.declaration_type || "Violation"}{violation.description ? ` — ${violation.description}` : ""}</figcaption></figure>)}</div>}
          </section>}
