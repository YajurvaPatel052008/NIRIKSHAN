...
const initialItems = [
  { id: "country-origin", label: "Country of Origin", value: "Not detected", confidence: 72, reason: "", correction: "" },
  { id: "consumer-care", label: "Consumer Care Details", value: "1800 123 4567", confidence: 78, reason: "", correction: "" },
  { id: "manufacture-date", label: "Month & Year of Manufacture", value: "03/2025", confidence: 81, reason: "", correction: "" },
  { id: "mrp", label: "MRP (Maximum Retail Price)", value: "₹149.00", confidence: 94, reason: "", correction: "" },
  { id: "net-quantity", label: "Net Quantity", value: "250 g", confidence: 97, reason: "", correction: "" },
];
...
export default function VerificationPage() {
  ...
  async function submitVerification() {
    ...
    const payload = {
      items: items.map((item) => ({
        item_id: item.id,
        action: item.action,
        corrected_value: item.action === "corrected" ? item.correction : null,
        reason: item.action === "corrected" ? item.reason || null : null,
      })),
    };
    try {
      const apiUrl = process.env.NEXT_PUBLIC_API_URL;
      if (apiUrl) {
        const response = await fetch(`${apiUrl}/api/inspections/demo-inspection/verify`, { method: "PATCH", headers: { "Content-Type": "application/json" }, body: JSON.stringify(payload) });
  ...
  return (
    <div className="min-h-screen bg-[#f4f8fa] text-slate-800">
      <aside className="fixed inset-y-0 left-0 hidden w-64 flex-col bg-[#0b304d] text-white lg:flex"><div className="flex h-[76px] items-center gap-3 border-b border-white/10 px-6"><span className="flex size-9 items-center justify-center rounded-lg bg-white/10"><ShieldCheck size={21} /></span><span className="text-lg font-bold tracking-[0.14em]">NIRIKSHA</span></div><nav className="space-y-1 px-4 pt-8"><Link href="/dashboard/inspector" className="flex items-center gap-3 rounded-md px-3 py-2.5 text-sm font-medium text-blue-100/65 hover:bg-white/5 hover:text-white"><ChevronLeft size={17} /> Dashboard</Link><Link href="/dashboard/inspector/new-inspection/compliance" className="flex items-center gap-3 rounded-md bg-white/10 px-3 py-2.5 text-sm font-medium text-white"><Flag size={17} /> Human Verification</Link></nav></aside>
